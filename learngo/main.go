...
type delivery struct {
	status       string
	onedelievery map[string]int // 한 번에 배송하는 물품의 뜻으로 생각
}

func newDelivery() delivery {
	d := delivery{}
	d.onedelievery = map[string]int{}
	return d
}
...
func main() {
	...
	for {
		...
		if menu == 1 { // 물건 구매
			...
			if itemchoice == 1 {
				buying(items, buyer, itemchoice, &numbuy)
			...
			for {
				...
				if bucketmenu == 1 {
					if canbuy {
						bucketBuying(items, buyer, &numbuy)
						break
					} else {
...
func bucketBuying(itm []item, byr *buyer, num *int) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("\n", r, "\n")
		}
...
func buying(itm []item, byr *buyer, itmchoice int, num *int) {
	defer func() {
		err := recover()
		fmt.Println(err)
